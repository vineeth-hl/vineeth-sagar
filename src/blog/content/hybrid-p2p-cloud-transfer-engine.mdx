...
downloads:
  - label: "Read Online"
    url: "/reports/Hybrid P2P-Cloud/hybrid-p2p-report.html"
...
|:---:|:---|:---:|:---|:---|:---|
| `GET` | `/ping` | No | — | `{"status": "ok", "message": "pong"}` | Health check |
| `GET` | `/metrics` | No | — | Prometheus text format | Auto-instrumented metrics |
| `POST` | `/api/auth/join` | No | `{"anonymous_name": "Hacker_4821"}` | `{"access_token": "eyJ...", "token_type": "bearer"}` | Anonymous session creation |
| `POST` | `/api/heartbeat` | Yes | `{}` | `200 OK` | Refresh Redis presence TTL to 60s |
| `GET` | `/api/users/online` | No | — | `[{"user_id": "uuid", "name": "Hacker_4821"}]` | List active peers |
| `POST` | `/api/upload` | Yes | `multipart/form-data` (file) | `{"url": "https://s3...presigned"}` | S3 cloud fallback upload |
...
|:---|:---:|:---|:---|
| `SECRET_KEY` | Yes | — | JWT HS256 signing secret |
| `REDIS_URL` | No | `None` (falls back to InMemoryStore) | Redis connection URI |
| `AWS_ACCESS_KEY_ID` | Yes | — | AWS IAM access key |
| `AWS_SECRET_ACCESS_KEY` | Yes | — | AWS IAM secret key |
| `AWS_REGION` | Yes | `eu-north-1` | AWS region for S3 |
| `S3_BUCKET_NAME` | Yes | — | Target S3 bucket name |
| `PORT` | No | `8000` / `3001` | Dynamic port (Railway/Render) |
